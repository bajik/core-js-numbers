...
function toNumber(value, def) {
  const convNumber = Number(value);
  return Number.isNaN(convNumber) ? def : convNumber;
}
...
function isPowerOfTwo(num) {
  ...
  const roundedLog = Math.round(logBase2);
  return logBase2 === roundedLog;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = x1 + x2 + x3;
  return Number.isInteger(sum) ? sum : sum.toPrecision(1);
}
...
function getCountOfOddNumbers(number) {
  let sum = 0;
  for (let i = 0; i < Math.abs(number); i += 2) {
    sum += 1;
  }
  return sum;
}
